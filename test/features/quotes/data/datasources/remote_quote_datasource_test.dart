...
void main() {
  ...
  final tQuoteListJson = json.encode({
    'results': tQuoteList.map((q) => q.toJson()).toList()
  });

  test('should perform a GET request on the URL', () async {
    // arrange
    when(() => mockHttpClient.get(any())).thenAnswer((_) async => http.Response(tQuoteListJson, 200));

    // act
    await dataSource.fetchQuotes();

    // assert
    verify(() => mockHttpClient.get(Uri.parse('https://api.quotable.io/quotes?limit=50')));
  });

  test('should return List<QuoteModel> when the response code is 200', () async {
    // arrange
    when(() => mockHttpClient.get(any())).thenAnswer((_) async => http.Response(tQuoteListJson, 200));
